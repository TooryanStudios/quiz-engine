...
const INPUT_SOCKETS = [
  { id: 'images_ref', label: 'Images Array', slot: 1 },
  { id: 'prompt', label: 'Prompt', slot: 2 },
  { id: 'audio', label: 'Audio', slot: 3 },
] as const

const OUTPUT_SOCKETS = [{ id: 'out-video', label: 'Video', slot: 2 }] as const

export const GenImagesToVideoNode = memo(function GenImagesToVideoNode({ id, data, isConnectable }: WorkflowBuilderNodeProps) {
  ...
  return (
    <WorkflowNodeFrame
      nodeId={id}
      kind="gen_images_to_video"
      title={data.label || 'Images to Video'}
      description=""
      icon={Images}
