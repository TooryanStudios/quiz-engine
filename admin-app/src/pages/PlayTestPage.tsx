...
export default function PlayTestPage() {
  ...
  const [resolvedGameId, setResolvedGameId] = useState<string>(gameId || 'clue-chain')

  useEffect(() => {
    setResolvedGameId(gameId || 'clue-chain')
  }, [gameId])
  ...
  useEffect(() => {
    ...
    getQuizById(quizId)
      .then((data) => {
        if (data?.gameModeId) {
          setResolvedGameId(data.gameModeId)
        }
        if (data?.themeId) {
  ...
  return (
    <div style={{ height: '100vh', width: '100vw', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '0.5rem', background: '#333', color: 'white', display: 'flex', gap: '1rem', alignItems: 'center', zIndex: 1000, position: 'relative' }}>
        ...
        <span style={{ fontSize: '0.85rem', opacity: 0.82 }}>Game: {resolvedGameId}</span>
         <select 
      ...
      <div style={{ flex: 1, position: 'relative' }}>
          <Play key={resolvedGameId + selectedPresetKey} gameId={resolvedGameId} theme={activeTheme as any} />
      </div>
    </div>
  );
}
