...
export function PacksPage() {
  ...
  const t = {
    ...
    subReqMsg: isAr ? 'هذا الاختبار متميز. يرجى ترقية حسابك لاستخدامه.' : 'This challenge is premium content. Please upgrade your account to use it.',
    upgradeNow: isAr ? 'ترقية الآن' : 'Upgrade now',
    cancel: isAr ? 'إلغاء' : 'Cancel',
    loadError: isAr ? 'فشل تحميل الاختبارات العامة.' : 'Failed to load public challenges.',
    cloneSuccess: isAr ? 'تم استنساخ الاختبار — يمكنك الآن تعديل نسختك.' : 'Challenge cloned — you can now edit your copy.',
    ...
    title: isAr ? 'مكتبة الاختبارات العامة' : 'Public Challenge Library',
    subtitle: (count: number) => isAr ? `${count} اختبار · تصفح، عاين أو استنسخ` : `${count} challenge${count !== 1 ? 's' : ''} · Browse, preview or clone`,
    all: isAr ? 'الكل' : 'All',
    mine: isAr ? 'خاصتي' : 'Mine',
    others: isAr ? 'الآخرين' : 'Others',
    searchPlaceholder: isAr ? 'بحث المواضيع، العناوين...' : 'Search topics, titles...',
    noQuizzesYet: isAr ? 'لا توجد اختبارات عامة بعد.' : 'No public challenges yet.',
    noResultsFound: isAr ? 'لم يتم العثور على أية نتائج توافق بحثك.' : 'No challenges matched your search.',
  ...
  const requiresSubscription = (quiz: QuizItem) => quiz.priceTier === 'starter' || quiz.priceTier === 'pro'
