...
export const TimelineSection: React.FC<TimelineSectionProps> = () => {
  ...
  const AUTO_COLLAPSE_THRESHOLD = HEADER_HEIGHT + 8;
  const AUTO_EXPAND_THRESHOLD = HEADER_HEIGHT + 24;
  ...
  const { bottomHeight, isResizing, handleMouseDown, handleTouchStart, setHeight } = useTimelineResize({
    overlays,
  });

  // Collapse handler
  const handleCollapseChange = React.useCallback((collapsed: boolean) => {
    if (!collapsed && bottomHeight <= AUTO_COLLAPSE_THRESHOLD) {
      setHeight(AUTO_EXPAND_THRESHOLD + 24);
    }
    setIsTimelineCollapsed(collapsed);
  }, [bottomHeight, setHeight]);

  // Auto-collapse when dragging near the minimum, and auto-expand when dragging back up.
  React.useEffect(() => {
    setIsTimelineCollapsed((prev) => {
      if (!prev && bottomHeight <= AUTO_COLLAPSE_THRESHOLD) {
        return true;
      }
      if (prev && bottomHeight >= AUTO_EXPAND_THRESHOLD) {
        return false;
      }
      return prev;
    });
