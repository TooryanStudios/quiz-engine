...
export const Timeline = forwardRef<TimelineRef, TimelineProps>(({ 
  ...
  const [internalGapRemovalEnabled, setInternalGapRemovalEnabled] = useState(false);
  const [snappingEnabled, setSnappingEnabled] = useState(true);
  const [hoverMarkerEnabled, setHoverMarkerEnabled] = useState(true);
  ...
  const {
    ...
    handleMouseMove,
    handleMouseLeave,
  } = useTimelineInteractions(timelineRef, zoomScale, hoverMarkerEnabled);
  ...
  const gapRemovalEnabled =
    externalGapRemovalEnabled !== undefined
      ? externalGapRemovalEnabled
      : internalGapRemovalEnabled;

  const handleGapRemovalToggle = useCallback((enabled: boolean) => {
    if (onGapRemovalEnabledChange) {
      onGapRemovalEnabledChange(enabled);
      return;
    }
    setInternalGapRemovalEnabled(enabled);
  }, [onGapRemovalEnabledChange]);

  const handleSeekTo = useCallback((timeInSeconds: number) => {
    clearTimelineMarkerPosition();
    onFrameChange?.(Math.round(timeInSeconds * fps));
  }, [onFrameChange, fps]);
  ...
  return (
    <div className="timeline-container bg-background flex flex-col h-full overflow-hidden">
      <TimelineHeader 
        ...
        onGapRemovalEnabledChange={handleGapRemovalToggle}
        snappingEnabled={snappingEnabled}
        onSnappingEnabledChange={setSnappingEnabled}
        hoverMarkerEnabled={hoverMarkerEnabled}
        onHoverMarkerEnabledChange={setHoverMarkerEnabled}
