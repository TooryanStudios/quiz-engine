...
export const useTimelineDragAndDrop = ({
  ...
  const snapToGridAndEdges = useCallback((
    ...
    // When snapping is disabled, return the raw value unconstrained
    if (!snappingEnabled) {
      return value;
    }
    ...
    // Also include the playhead (currentTime) as a snap target
    const edgePositions: number[] = [currentTime];
    ...
    return gridSnapped;
  }, [tracks, snappingEnabled, currentTime]);
