...
export const useTimelineInteractions = (
  ...
  zoomScale: number = 1,
  ghostMarkerEnabled: boolean = true
) => {
  ...
  const hideGhostMarker = useCallback(() => {
  ...
  useEffect(() => {
    if (!ghostMarkerEnabled) {
      hideGhostMarker();
    }
  }, [ghostMarkerEnabled, hideGhostMarker]);
  ...
  }, [ghostMarkerEnabled, isDragging, timelineRef, zoomScale]);
