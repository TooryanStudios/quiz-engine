  ...
  const generateMarkers = () => {
    ...
    for (let time = 0; time <= totalDuration; time += minorInterval) {
      ...
      if (!isMajor) {
        markers.push(
          ...
          >
            <div className="w-px bg-gray-400 dark:bg-gray-400 h-1 opacity-60" />
          </div>
        );
      }
    }
    ...
    for (let time = 0; time <= totalDuration; time += interval) {
      ...
      markers.push(
        ...
        >
          <div className="w-px bg-gray-400 dark:bg-gray-400 h-2" />
          <span className="text-[10px] mt-0.5 select-none whitespace-nowrap text-text-secondary leading-none">
