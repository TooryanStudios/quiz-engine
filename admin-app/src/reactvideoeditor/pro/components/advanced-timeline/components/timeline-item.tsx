...
export const TimelineItem: React.FC<TimelineItemProps> = ({ 
  ...
  const pendingMouseDragRef = useRef<{
    startX: number;
    startY: number;
    action: "move" | "resize-start" | "resize-end";
  } | null>(null);
  ...
  const getDragActionFromPointer = useCallback((clientX: number): "move" | "resize-start" | "resize-end" => {
    const rect = itemRef.current?.getBoundingClientRect();
    if (!rect) return "move";
    ...
    if (!handlesVisible) {
      ...
      if (relativeX >= itemWidth - resizeHandleWidth) {
        return "resize-end";
      }
    }

    return "move";
  }, [isSelected, selectedItemIds.length, splittingEnabled, onDragStart]);
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    if (!onDragStart || splittingEnabled) {
    ...
    pendingMouseDragRef.current = {
      startX: e.clientX,
      startY: e.clientY,
      action: getDragActionFromPointer(e.clientX),
    };
    dragStartedRef.current = false;

    const onWindowMouseMove = (moveEvent: MouseEvent) => {
      const pending = pendingMouseDragRef.current;
      if (!pending || dragStartedRef.current) return;

      const deltaX = Math.abs(moveEvent.clientX - pending.startX);
      const deltaY = Math.abs(moveEvent.clientY - pending.startY);
      ...
      onDragStart(item, pending.startX, pending.startY, pending.action, selectedItemIds);
    };
  ...
  return (
    <ContextMenu onOpenChange={onContextMenuOpenChange}>
      <ContextMenuTrigger asChild>
        <div
          ref={itemRef}
          className={`timeline-item group absolute top-1/2 transform -translate-y-1/2 rounded flex items-center justify-center text-xs font-light text-white shadow-sm hover:shadow-md transition-shadow select-none overflow-hidden touch-none ${getCursorStyle().className} ${isSelected ? 'border border-white/70 ring-1 ring-white/40' : 'border border-white/20'}`}
          style={{
            left: `${leftPercentage}%`,
