...
export const TimelineGuidelines: React.FC<TimelineGuidelinesProps> = ({
  ...
  // Determine if the drag position is snapping to the playhead
  const SNAP_TOLERANCE = 0.05;
  const isSnappingToPlayhead = isDragging && currentDragPosition !== null && currentDragPosition !== undefined && currentTime !== undefined && (
    Math.abs(currentDragPosition.start - currentTime) <= SNAP_TOLERANCE ||
    Math.abs(currentDragPosition.end - currentTime) <= SNAP_TOLERANCE
  );

  if (!isDragging || (guidelines.length === 0 && !isSnappingToPlayhead)) {
    return null;
  }
  ...
  return (
    <div className="absolute inset-0 pointer-events-none" style={{ zIndex: 55 }}>
      ...
      {isSnappingToPlayhead && currentTime !== undefined && (
        <div
          className="absolute"
          style={{
