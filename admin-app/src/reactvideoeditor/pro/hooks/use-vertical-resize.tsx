...
export const useVerticalResize = (options: UseVerticalResizeOptions = {}): UseVerticalResizeReturn => {
  ...
  const latestHeightRef = useRef(bottomHeight);
  ...
  const scheduleHeightUpdate = useCallback((nextHeight: number) => {
    ...
    rafIdRef.current = window.requestAnimationFrame(() => {
      ...
      if (pending !== latestHeightRef.current) {
        setBottomHeight(pending);
      }
    });
  }, []);
  ...
  const handleMouseMove = useCallback(
    ...
    [isResizing, minHeight, maxHeight, scheduleHeightUpdate]
  );
  ...
  useEffect(() => {
    latestHeightRef.current = bottomHeight;
  }, [bottomHeight]);
