...
// Cache for font info to avoid redundant work in editor/runtime environments.
...
const loadFontInfo = async ({
  ...
  // In the editor/Vite runtime we intentionally skip the API lookup.
  // Text layers fall back to browser-available fonts and SSR/rendering passes in real font info.
  if (!fontInfoPromiseCache[fontFamily]) {
    fontInfoPromiseCache[fontFamily] = Promise.resolve(getRobotoFontInfo());
  }
...
export const loadFontFromTextItem = async (item: LoadFontInfo): Promise<void> => {
  try {
    ...
    // In this Vite integration, skip dynamic Google font loading from font info
    // to avoid subpath resolution issues with @remotion/google-fonts.
    // The editor gracefully falls back to browser-available fonts.
    void fontInfo;
    void variant;
    void fontWeight;
  } catch (error) {
