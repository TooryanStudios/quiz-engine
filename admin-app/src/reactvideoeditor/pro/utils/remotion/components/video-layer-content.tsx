...
import { refreshMediaBlobUrl } from "../../general/indexdb";
...
export const VideoLayerContent: React.FC<VideoLayerContentProps> = ({
  ...
  const [resolvedVideoSrc, setResolvedVideoSrc] = useState<string>(overlay.src);
  ...
  useEffect(() => {
    ...
    const handleError = async (error: Event) => {
      if (
        !attemptedRecoveryRef.current &&
        overlay.localMediaId &&
        resolvedVideoSrc.startsWith("blob:")
      ) {
        attemptedRecoveryRef.current = true;
        const refreshedUrl = await refreshMediaBlobUrl(overlay.localMediaId);
        if (refreshedUrl && !disposed) {
          setResolvedVideoSrc(refreshedUrl);
          continueRender(handle);
          return;
        }
      }
  ...
  }, [overlay.localMediaId, overlay.src, resolvedVideoSrc]);
  ...
  const _onVideoFrame = useCallback(
    (videoFrame: CanvasImageSource) => {
